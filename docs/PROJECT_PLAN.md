...
- 2026-09-24: A restored Supabase session never renders budget data until Zenify is unlocked. The app locks on launch and whenever it leaves the foreground; native builds use strong device biometrics with password sign-in as the fallback, while the web PWA always requires the password fallback because browser apps cannot invoke native Face ID directly.
